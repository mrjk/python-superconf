...
from superconf.lib.anchors import FileAnchor, PathAnchor
...
def test2_nested():
    ...
    pd_inventory = PathAnchor("inventory/", parent=pd)
    pd_conf = PathAnchor("../../common_conf", parent=pd)
...
def test3_parents():
    ...
    pd1 = PathAnchor("../../common_conf", parent=pd0)
    pd2 = PathAnchor("inventory/", parent=pd1)
    ...
    ret = {
        "pd0a": PathAnchor("subconf/", parent=pd0).get_dir(),
        "pd1a": PathAnchor("subconf/", parent=pd1).get_dir(),
        "pd2a": PathAnchor("subconf/", parent=pd2).get_dir(),
    }
    pprint(ret)
...
def test4_modes():
    ...
    # project_dir = "fake/prj"
    pd0 = PathAnchor(project_dir)
    pd1 = PathAnchor("../../common_conf", parent=pd0, mode="abs")
    pd2 = PathAnchor("inventory/", parent=pd1, mode="rel")

    # pprint (pd0.__dict__)
    # pprint (pd1.__dict__)
    # pprint (pd2.__dict__)

    pd0a = PathAnchor("subconf/", parent=pd0)
    pd1a = PathAnchor("subconf/", parent=pd1)
    pd2a = PathAnchor("subconf/", parent=pd2)
...
def test5_files():
    ...
    pd0a = FileAnchor("subconf/myfile1.yml", parent=pd0)
    pd1a = FileAnchor("subconf/myfile2.yml", parent=pd1)
    pd2a = FileAnchor("subconf/myfile3.yml", parent=pd2)
